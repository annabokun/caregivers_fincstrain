...
write.csv(ACC_cg_coreside, "/Users/annabokun/Google Drive/My Drive/UT-Austin Postdoc/Projects/Caregivers - Housing Health Paradox/Replication Files/Data/ACC_cg_coreside.csv")
  ...
  #### M1:) Baseline model: co-residence on financial strain --------------------------- 
    ...
    #### M2:) HOH on financial strain, among co-residing ACCs --------------------------- 
  ...
  #### M3:) co-residence*health status of the parent --------------------------- 
        M3 <- glm(cg_finc_strain ~ cg_coreside*cr_health +  
                    cg_age_cat + 
                    cg_female + 
                    cg_marital_stat + 
                    cg_educ_cat + 
        ...
        M3_table <- tbl_regression(
          ...
          modify_caption("**M4. Logistic Regression of Financial Strain on Co-residence x Health Status of Parent**")
    ...
    #### Merge them into a single document --------------------------- 
        merged_tables <- tbl_merge(
          tbls = list(M1_table, M2_table, M3_table),
          tab_spanner = c("M1_table", "M2_table", "M3_table")
        )
      ...
      ### 4/25/2025 update 
      ## Can't estimate anything with CG intensity, since CG ADL hours is ~27% missing (main input for CG intensity measure)
        
        #### M3:) co-residence*CG intensity on financial strain --------------------------- 
