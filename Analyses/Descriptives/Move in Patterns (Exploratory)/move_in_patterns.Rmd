...
# This creates the variables I'll use throughout the document
ACC <- ACC %>%
  mutate(
    ...
    move_in_type = case_when(
      ...
      MOVEIN7 == 1 &                                  # Anyone moved in with you since last contact?
        (WHOMOV71 %in% 3:4) ~ 5,                      # Children or children-in-law moved in
    ...
    move_in_type_label = case_when(
      ...
      move_in_type == 5 ~ "General move-in by adult child (unspecified reason)",
    ...
    enriched_typology = case_when(
      ...
      move_in_type == 1 & cg_HOH == 0 ~ "Parent moved in for care (caregiver is not HOH)",
      
      # Adult child moved in for caregiving + caregiver is head of household
      move_in_type == 2 & cg_HOH == 1 ~ "Adult child moved in to care (caregiver is HOH)",
      
      # Adult child moved in for caregiving + caregiver is not head of household
      move_in_type == 2 & cg_HOH == 0  ~ "Adult child moved in to care (caregiver is not HOH)",
      
      # Adult child moved in for caregiving + parent owns home
      move_in_type == 2 & LL76 == 1 ~ "Adult child moved in to care (parent owns home)",
      
      # Adult child moved in for caregiving + parent doesn't own home
      move_in_type == 2 & LL76 == 2 ~ "Adult child moved in to care (parent doesn't own home)",
      ...
      move_in_type == 5 & cg_coreside == 1 ~ "Adult child moved in (general)",
...
format_table(
  adult_child_basic,
  caption = "Move-In Types for Adult Child Caregivers",
...
6.) Marriage Status as a Buffer: Adult children who had parents move in show dramatically higher marriage rates (90%) than adult children who moved in with  their parents for caregiving (53.8%), suggesting that spousal support may influence living arrangement decisions.
