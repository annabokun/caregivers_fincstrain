...
This document presents analysis of move-in patterns among Latino caregivers and care recipients from the Hispanic Established Populations for the Epidemiologic Study of the Elderly (HEPESE) data. The analysis examines these patterns from both the care recipient's and caregiver's perspectives, with a focus on who moves in with whom and for what reasons.
...
```{r typology}
# Create typology dataframe with counts
move_in_typology_counts <- ACC %>%
  mutate(
    move_in_typology = case_when(
      cg_coreside == 1 & cg_HOH == 1 & LL76 == 2 ~ "Care recipient/parent likely moved in with caregiver",
      cg_coreside == 1 & cg_HOH == 0 & LL76 == 1 ~ "Caregiver likely moved in with care recipient/parent",
      TRUE ~ "Other co-residence arrangement"
    )
  ) %>%
...
format_table(
  ...
  caption = paste0("Inferred Move-In Typology (N=", sum(move_in_typology_counts$count), ")"),
...
```{r financial-strain}
# Financial strain by household structure and move-in typology
# First recreate the typology variable directly on the ACC dataframe
...
## 2.4. Caregiver Characteristics by Living Arrangement

```{r caregiver-characteristics}
# Analyze caregiver characteristics by living arrangement type
caregiver_by_type <- ACC %>%
  mutate(
    move_in_typology = case_when(
      cg_coreside == 1 & cg_HOH == 1 & LL76 == 2 ~ "Care recipient/parent likely moved in with caregiver",
      cg_coreside == 1 & cg_HOH == 0 & LL76 == 1 ~ "Caregiver likely moved in with care recipient/parent",
...
# 3. Move-In Typologies for Adult Child Caregivers

This section presents a more refined analysis of move-in patterns, specifically focusing on adult child caregivers.
...
adult_child_dementia <- ACC %>%
  ...
  summarize(
    ...
    n_dementia_valid = sum(!is.na(cr_dementia)),  # Count valid cases
...
1. **Who Moves In**: The analysis shows that adult children (including step-children & children in-law) constitute the largest group of individuals who move in with care recipients, followed by spouses and grandchildren.

2. **Reasons for Moving In**: I identified multiple motivations for move-ins:
   - Providing care for the parent
...
```{r final-figure, fig.width=14, fig.height=6}
...
summary_data <- ACC %>%
  ...
  summarize(
    count = n(),
    pct_coresident = 100 * mean(cg_coreside == 1, na.rm = TRUE),
